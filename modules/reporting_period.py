...
def period_presets(year):
    ...
    확정 프리셋은 분기(법인 기준)입니다. 개인 일반과세자처럼 확정신고가
    반기 전체를 덮는 경우는 상반기/하반기를 고릅니다. 라벨의 월 범위가
    실제 적용 범위이므로 혼동 여지가 없도록 라벨에 항상 월을 표기합니다.
    """
    return {
        "상반기 (1~6월)": (f"{year}-01-01", f"{year}-06-30"),
        "하반기 (7~12월)": (f"{year}-07-01", f"{year}-12-31"),
        "1기 예정 (1~3월)": (f"{year}-01-01", f"{year}-03-31"),
        "1기 확정 (4~6월)": (f"{year}-04-01", f"{year}-06-30"),
        "2기 예정 (7~9월)": (f"{year}-07-01", f"{year}-09-30"),
        "2기 확정 (10~12월)": (f"{year}-10-01", f"{year}-12-31"),
    }
